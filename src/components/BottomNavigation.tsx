import { Home, Search, Heart, User, Download, RefreshCw } from 'lucide-react';
import { NavLink, useLocation } from 'react-router-dom';
import { useAuth } from '@/hooks/useAuth';

const navigationItems = [
  { title: 'Home', url: '/', icon: Home, action: 'navigate' },
  { title: 'Saved', url: '/saved', icon: Heart, action: 'navigate' },
  { title: 'Downloads', url: '/downloads', icon: Download, action: 'navigate' },
  { title: 'Profile', url: '/profile', icon: User, action: 'navigate' },
];
        ...
        {navigationItems.map((item) => {
          ...
          return (
            <NavLink
              key={item.title}
              to={item.url}
              onClick={item.title === 'Home' ? handleHomeRefresh : undefined}
